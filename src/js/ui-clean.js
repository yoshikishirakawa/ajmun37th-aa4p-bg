...
(function () {
  ...
  function setupHeaderControls() {
    ...
    if (fontSizeSelect) {
      const currentSize = localStorage.getItem(STORAGE_KEYS.fontSize) || 'medium';
    ...
    const headerHTML = `
      <div class="scroll-settings-header" id="scroll-settings-header">
        <div class="scroll-settings-header-left">
          <span class="scroll-settings-header-title">平和への課題：補遺</span>
        </div>
    ...
    const fontSizeObserver = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'data-font-size') {
          const currentSize = document.body.getAttribute('data-font-size');
  ...
  // Remove sidebar search and use only button search
  function setupSidebarSearch() {
    ...
    // Remove Quarto's default search UI and metadata so we don't trigger file:// warnings
    const defaultResultsPanel = document.getElementById('quarto-search-results');
    if (defaultResultsPanel) {
      defaultResultsPanel.remove();
    }
    ...
    // Remove everything from sidebar search
    sidebarSearch.innerHTML = '';
    sidebarSearch.style.display = 'none'; // Hide the entire search container

    // Search is now only available via the scrollable header and keyboard shortcuts
